...
class CrudCode extends CCodeModel
{
	...
	public function generateInputField($modelClass,$column)
	{
		...
		elseif(stripos($column->dbType,'text')!==false)
			return "CHtml::activeTextArea(\$model,'{$column->name}',array('class'=>'form-control', 'rows'=>6, 'cols'=>50))";
		else
		{
			...
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "CHtml::{$inputField}(\$model,'{$column->name}',array('class'=>'form-control', 'size'=>$size,'maxlength'=>$maxLength))";
			}
		}
	}
